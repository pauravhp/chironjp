...
| --- | --- | --- | --- | --- |
| Disposable VPS-equivalent, fresh home | Clean supported Linux VM/container; new unprivileged user and empty home; no credentials | Clone the release candidate, follow only the published README, install, start, stop, then uninstall/recreate | Full redacted command transcript and versions; health/output expected by the README; inventory showing writes remain in documented checkout/runtime paths | Same-host isolated-home lifecycle PASS at `f725144`: anonymous clone/install, setup rerun, fictional ingress, unauthenticated Review `401`, exact stop, recoverable workspace replacement, and clean second setup/ingress/stop. This did not use a fresh OS, VPS, browser, Tailor, or phone, so full row remains not run. |
| Existing-Hermes non-clobber | Disposable host with a functioning, non-production Hermes installation and canary state; snapshot its owned units, containers, ports, paths, hashes, and health without opening secrets | Install/start/use/stop Chiron with its documented defaults, then remove only Chiron-owned state | Before/after inventory has no Hermes delta; Hermes health and canary are unchanged; no shared unit/container/volume/user/port/config name | Local isolated canary PASS: harmless authenticated Hermes calls before and after the scoped Chiron lifecycle both returned the exact canary response with completed usage and exit `0`; stable config and skill manifests were identical; dedicated Chiron profiles, credentials, listener, and processes were absent after cleanup. Hermes normally removed one notice file, one lock file, and two regenerable cache files during the calls, so the whole disposable home was not byte-identical. No shared production runtime was used. |
| Fictional profile and resume render | Fresh runtime state containing only a clearly fictional profile and resume bank | Select the example role/profile and render each documented output format | Human-reviewed render has correct example identity, role choice, page layout, links, and no host path, secret, hidden real identity, or overflow | Local actual-Tailor PASS: stock Hermes ran GPT-5.6 Terra at high reasoning for the fictional profile; request usage completed without failure, artifact hashes matched the selection/validation/vision contract, and a controller inspected the exact one-page PNG for identity, project choice, layout, links, and clipping. This was not a fresh-OS deployment proof. |
| Source ingestion to review/demo | Controlled public source fixture plus a separately labelled live-source check if authorized | Ingest, normalize canonical employer/application identity, deduplicate, rank/select, and open the retained item in the review UI | Trace connects source identity to normalized record and review view; rerun is idempotent; no submission/network delivery occurs | Local integrated fictional-fixture PASS at `9258d22`: the earlier source/package and actual Tailor artifact were handed to stock Hermes running `z-ai/glm-5.3-flash` at high reasoning. The worker retained the exact application target at guarded Review and exited `0`; both worker readback and an independent post-exit CDP readback verified the browser File object as `tailored-resume.pdf`, 31,989 bytes, with the exact admitted SHA-256. Submit remained untouched and the fixture recorded zero final activations. This used a loopback fictional employer, not a live employer or external deployment, and no application was submitted. |
