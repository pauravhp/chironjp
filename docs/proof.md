...
| Source ingestion to review/demo | Controlled public source fixture plus a separately labelled live-source check if authorized | Ingest, normalize canonical employer/application identity, deduplicate, rank/select, and open the retained item in the review UI | Trace connects source identity to normalized record and review view; rerun is idempotent; no submission/network delivery occurs | Partial local proof: an authorized live-source scan normalized 50 rows and an immediate rerun inserted none; the fictional source reached the bound package and actual Tailor artifact. At `13221990`, stable per-worker Browser Harness preflight retained the exact target and passed `Runtime.evaluate`, but the one stock-Hermes GLM attempt received Nous HTTP `429` after its built-in retries before any browser action. No file was selected, no Review was published, and no final action occurred; this row remains incomplete. |
